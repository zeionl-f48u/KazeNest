...
> 本段是**给 AI 助手的工作提示词**：在本仓库 `src-tauri/` 下工作时，AI 只做三件事——
> **识别问题、添加注释、维护文档**；**不得修改任何后端实现内容**。

**职责范围（仅允许做这些）**

1. **识别问题**：通读代码、骨架注释、配置与本文档，发现并归类问题——
   安全隐患、设计冲突、契约不一致、平台差异、权限遗漏、文档过期等。
2. **添加注释**：为现有文件补充/完善文档注释与行内说明；允许加入
   `// TODO(ai): …` / `// FIXME(ai): …` 标记（只标记，不修）。
3. **维护文档**：更新本 README（目录、表格、章节、附录清单）、修正骨架注释中的
   "计划实现"描述、维护「附录：AI 问题记录清单」。

**禁止事项（红线）**

1. 不修改后端实现：不写/改业务逻辑、不启用 `mod`、不新增函数实现、不改已有函数体。
2. 不动配置与权限：不改 `Cargo.toml`、`tauri.conf.json`、`capabilities/*.json`、`build.rs`。
3. 不动前端：不修改 `src/`（Vue/TS）下任何文件，文档中描述契约除外。
4. 不执行破坏性操作：不删文件、不改依赖版本、不 `cargo update`、不改变目录结构。
5. **唯一例外**：任务明确要求"实现/修改某功能"时，按第 14 节路线图执行；
   否则一律默认只读（代码只读，注释与文档可写）。

**工作流程**

1. 审查目标范围，将问题分为五类：安全 / 设计 / 契约 / 平台 / 文档；
2. 在问题所在处补注释标记（`// TODO(ai): 问题描述`），**不修代码**；
3. 在「附录：AI 问题记录清单」追加条目（编号、位置、类型、描述、建议、状态）；
4. 若问题涉及命令表 / 事件表 / 权限表 / 依赖表，同步更新对应文档章节；
5. 汇报结果：列出"新增的注释 + 文档变更 + 记录的问题编号"，**不得声称已修复问题**。

**输出要求**

- 注释与文档一律中文；问题描述具体到文件、行号或章节，避免泛泛而谈；
- 不写 emoji；保持章节编号稳定；表格只追加行，不重排既有行；
- 无法判断某改动是否属于"实现范畴"时，先询问，不擅自行动。

> 任何越过职责范围的行为（改逻辑 / 改配置 / 改前端）视为无效改动，必须回退。
...
> 由 AI 审查时追加，**记录问题而非修复问题**（修复由人执行）。
> 编号规则：`AI-序号`；类型：安全 / 设计 / 契约 / 平台 / 文档；
> 状态：`待确认` → `已确认` → `已修复`（修复时由人更新状态并注明提交号）。

| 编号 | 位置 | 类型 | 问题描述 | 建议 | 状态 |
|---|---|---|---|---|---|
| AI-001 | `commands/fs.rs`（骨架注释） | 安全 | 示例：删除命令未定义"回收站不可用"时的降级策略 | v0.3 设计时补充：拒绝执行并返回结构化错误 | 待确认 |
