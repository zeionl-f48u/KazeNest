...
/** 同义别名（与 ICONS 表解耦，避免重复维护同一份路径） */
export function Icon({ name, size = 16, color, className, style }: IconProps) {
  const bare = name.replace(/^(pi\s+)?pi-?/, '')
  const path = ICONS[ALIASES[bare] ?? bare] ?? ICONS['dot']
  const px = typeof size === 'number' ? `${size}px` : size
  return (
    <svg
      className={className ? `kn-icon ${className}` : 'kn-icon'}
      style={{ width: px, height: px, ...(color ? { color } : null), ...style }}
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth={1.8}
